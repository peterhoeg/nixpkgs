...
let
  ...
  smartdOpts = { name, ... }: {
    ...
    options = {
      ...
      timeout = mkOption {
        ...
        type = types.int;
        description = ''
          Set SCT Error Recovery Control timeout in seconds for use in raid configurations.

          Values are as follows:
            -1 = do not touch, leave at default
             0 = disable SCT ERT
             7 = default in consumer drives (typically)
  ...
  deviceTimeout = d:
    lib.optionalString (d.timeout > -1) ''
      ${pkgs.smartmontools}/bin/smartctl -l scterc,${d.timeout}0,${d.timeout}0 --silent errorsonly ${d.device}
    '';
