...
let
  ...
  clientEnv = pkgs.buildEnv {
    name = "sensu-env";
    pathsToLink = [ "/bin" ];
    paths = (with pkgs; [ coreutils gawk ]
      ...
      ++ lib.optionals pgm.windows [ sensu-check-wmiplus ]
      ++ cfg.dependencies
  );};
      ...
      services = {
        ...
        in lib.mkIf transportIsRabbitMQ {
          ...
          script = ''
            ...
            export PATH=${lib.makeBinPath (with pkgs; [ coreutils gnused rabbitmq-server ])}:$PATH
