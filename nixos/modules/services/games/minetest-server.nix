...
let
  inherit (lib)
    concatStrings escapeShellArgs getExe getLib makeLibraryPath mapAttrsToList
  ...
  flags =
    let
      flag = val: name:
        optionals (val != null) [ "--${name}" "${toString val}" ];
  ...
  pkg = pkgs.symlinkJoin {
    inherit (pkgs.minetest) name version;
    paths = [ pkgs.minetest ] ++ cfg.addons;
  };
...
in
{
  ...
  config = mkIf cfg.enable {
    systemd.services.minetest-server = {
      ...
      serviceConfig = {
        ExecStart = "${getExe pkg} ${escapeShellArgs flags}";
        DynamicUser = true;
