...
let
  ...
  # mod security realpaths every read, so never link below a mod's own directory
  addonsSetup =
    pkgs.resholve.writeScriptBin "minetest-addons-setup"
      ...
      (
        ''
          ...
          _link() {
            for type in "$1"/share/luanti/*/; do
              dest="${dir}/.minetest/$(basename "$type")"
              mkdir -p "$dest"
              for addon in "$type"*/; do
                ln -sfn "''${addon%/}" "$dest/$(basename "$addon")"
              done
            done
          }

          mkdir -p ${dir}/.minetest
          find ${dir}/.minetest -mindepth 2 -maxdepth 2 -type l -lname '/nix/store/*' -delete
        ''
