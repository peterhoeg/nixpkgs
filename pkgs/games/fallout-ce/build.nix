...
let
  ...
  script = resholve.writeScriptBin pname
    {
      scripts = [ "bin/${pname}" ];
      interpreter = runtimeShell;
      inputs = [ "${binary}/libexec" coreutils findutils xorg.lndir ];
    ...
    ''
      ...
      if [ "''${SDL_VIDEODRIVER:-}" = "wayland" ]; then
        echo "There are issues running with SDL_VIDEODRIVER=wayland. You may want to unset this."
        notice=1
      fi
  ...
  binary = stdenv.mkDerivation {
    ...
    buildInputs = [ SDL2 fpattern ] ++ buildInputs;
...
symlinkJoin {
  name = "${pname}-${version}";
  paths = [ binary desktopItem script ];
  meta = meta';
}
