{ stdenv, lib, fetchFromGitHub, fetchpatch, autoreconfHook, runtimeShell
, coreutils, gnugrep, gnused, lm_sensors, net_snmp, openssh, openssl, perl
, dnsutils, libdbi, mysql, zlib, openldap, procps }:

let
  binPath = lib.makeBinPath [ coreutils gnugrep gnused lm_sensors net_snmp procps ];

  generic = { version, rev, sha256, patches ? [] }: stdenv.mkDerivation rec {
    ...
    postPatch = ''
      ...
      sed -i configure.ac \
        -e 's|^DEFAULT_PATH=.*|DEFAULT_PATH=\"/run/wrappers/bin:\$out/bin:${binPath}\"|'
    ...
    buildInputs = [
      dnsutils libdbi mysql net_snmp openldap openssh openssl perl procps zlib
    ];
...
in
{
  ...
  monitoring-plugins-unstable = generic {
    version = "2.2.0.20190812";
    rev = "5f4182f5da5ae6151eb960a0458571a88017df89";
    sha256 = "15gw2wnk8wrb9f9inyh8rlsm83w1nqdbixncss5dml23i9laddg5";
  };
}
