...
let
  ...
  glplugin = buildPerlPackage rec {
    name = "check-glplugin-${version}";
    version = "20170802";
  ...
  in perl.stdenv.mkDerivation rec {
    ...
    meta = with stdenv.lib; {
      inherit description;
      inherit (sensu-check-glplugin) homepage maintainer;
    };
  };
