...
, monitoring-plugins
, wmiClient ? wmic-bin
, wmic-bin ? null, py-wmi-client ? null }:
...
in stdenv.mkDerivation rec {
  ...
  propagatedBuildInputs = with perlPackages; [
    ClassSingleton ConfigIniFiles DateTime DateTimeLocale DateTimeTimeZone
    ModuleImplementation ModuleRuntime NumberFormat ParamsValidate TryTiny
    BHooksEndOfScope namespaceautoclean namespaceclean VariableMagic SubExporterProgressive
    PackageStash Specio EvalClosure MROCompat RoleTiny DevelStackTrace
    SubIdentify ParamsValidationCompiler ExceptionClass ClassDataInheritable
    FileShareDir ClassInspector
  ];
  ...
  postPatch = ''
    substituteInPlace check_wmi_plus.pl \
      --replace /usr/bin/wmic                      ${wmiClient}/bin/wmic \
